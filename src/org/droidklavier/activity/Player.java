...
public class Player extends FragmentActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mButtonQuietStatus.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String mode = (((ToggleButton) v).isChecked()) ? RC.QUIET_STATUS_QUIET
						: RC.QUIET_STATUS_ACOUSTIC;
		...
		if (mDroidklavier.isConnected()) {
			setActionBarStatus(getString(R.string.connected)
					+ mDroidklavier.getConnectedAddr());
		} else {
	...
	public void connectionSuccess() {
		...
		setActionBarStatus(String.format(getString(R.string.connected),
				connectedAddr));
		infoMessage(String.format(getString(R.string.connection_success),
				connectedAddr));
	...
	private void startSongListActivity(String source, Album album) {
		...
		if (mDroidklavier.isConnected() && checkSource(source) && !mBrowsing) {
			...
			intent.putExtra(Player.SOURCE,
					source.toLowerCase(Locale.getDefault()));
	...
	private class UpdateSongTask extends AsyncTask<Integer, Void, Song> {
		...
		protected Song doInBackground(Integer... params) {
			...
			if (mSource.equals(DAO.SEARCH_RESULT)) {
				song = mDroidklavier.getDAO().getSearchResultSong(selSongNo,
						albumId);
			} else {
				song = mDroidklavier.getDAO().getSong(mSource, albumId,
						selSongNo);
			}
